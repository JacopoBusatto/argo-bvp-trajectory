...
_This document is part of the preprocess documentation set._
...
3. Flag the cycle as:
   - `valid_for_bvp = True` if parking is sampled above the configured threshold,
...
5. Encode which other phases are attendible (sufficient IMU samples) and pass them through to BVP-ready products. Non-attendible phases (descent, ascent, profile, surface, etc.) are assumed vertical by the solver (zero horizontal displacement), anchored to parking start/end or surface fixes as appropriate.
6. Record per-phase sampling counts/flags in outputs:
   - `parking_n_obs`, `parking_attendible` (parking is mandatory gate)
   - `<phase>_n_obs`, `<phase>_attendible` for other detected phases (e.g., ascent, descent_to_profile, profile_drift, surface, in_air, grounded, other)
7. **Not** perform any trajectory integration. 
...
The BVP solver operates exclusively on cycles flagged as valid by the preprocessing (parking attendible).

It:
- integrates the equations of motion over phases that have sufficient IMU sampling (parking mandatory, other phases attendible when above threshold),
